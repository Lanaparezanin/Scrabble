...
public class SleepBeige implements ScrabbleAI {
    ...
    private class Move implements Comparable {
        ...
        public int compareTo(Object o) {
            SleepBeige.Move other = (SleepBeige.Move) o;
    ...
    private ScrabbleMove getTopScoreMove(ArrayList<Character> _hand, GateKeeper gateKeeper) {
        ...
        while (!possibleMoves.empty()) {
            ...
            for (String _word : out) {
                ...
                ArrayList<Character> tmpHand = new ArrayList<Character>();
                for (char c : _hand) {
                    tmpHand.add(c);
                }
                for (int i = 0; i < _word.length(); i++) {
                    ...
                    if (c != ' ') {
                        tmpHand.remove((Character)Character.toLowerCase(c));
                    }
                }
                int variance = 0;
                for (char c : tmpHand) {
                ...
                try {
                    ...
                    int score = gateKeeper.score(_word, hLocation, Location.HORIZONTAL) - variance;
                    if (maxScore < score) {
                        word = _word;
                ...
                try {
                    ...
                    int score = gateKeeper.score(_word, vLocation, Location.VERTICAL) - variance;
                    if (maxScore < score) {
                        word = _word;
